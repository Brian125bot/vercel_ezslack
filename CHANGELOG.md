...
* **Workspace/Channel Tool Policy Profiles.** Added a `tool_policies` table (migration 14) and admin endpoints (`GET`/`PUT` `/api/agent/tool-policy`, `DELETE /api/agent/tool-policy/:id`) to restrict which tools an agent run may use, scoped per workspace or per channel. `resolveAllowedTools()` in `policy.ts` resolves the effective policy once per run: a channel-level row wins outright over a workspace-level row (no merge), falling back to the workspace-level row, and to fully unrestricted (`null`, identical to pre-existing behavior) when neither exists. An unrecognized `profile` value denies all tools rather than silently falling back to unrestricted. The resolved allow-list is threaded through the single-shot planner (`createPlan`), the ReAct loop's tool declarations and per-call dispatch (`reactLoop.ts`), the plan-mutation tool lookups (`planMutation.ts`), and `executor.ts`, and is enforced at every tool-lookup site via new `toolsRegistry.getScoped()`/`getAllowed()` helpers — not just at the model's declared-tools list — so a hallucinated or adversarial direct tool call is rejected the same way a plain unregistered tool name is. Policy denials are logged as distinct audit event types (`tool.policy_denied`, `step.policy_denied`, `plan.mutation.policy_denied`) for observability without changing the model-facing error text.
