...
async reinsertScheduledTrigger(trigger: ScheduledTrigger, nextRunAt: Date | null): Promise<void> {
  ...
  async getChannelPolicy(workspaceId: string, channelId: string): Promise<ToolPolicy | null> {
    const rows = await query<ToolPolicy>(
      `SELECT * FROM tool_policies WHERE workspace_id = $1 AND channel_id = $2`,
      [workspaceId, channelId]
    );
    return rows[0] || null;
  },

  async getWorkspacePolicy(workspaceId: string): Promise<ToolPolicy | null> {
    const rows = await query<ToolPolicy>(
      `SELECT * FROM tool_policies WHERE workspace_id = $1 AND channel_id IS NULL`,
      [workspaceId]
    );
    return rows[0] || null;
  },
  ...
  async listToolPolicies(workspaceId: string): Promise<ToolPolicy[]> {
    return query<ToolPolicy>(
      `SELECT * FROM tool_policies WHERE workspace_id = $1 ORDER BY created_at ASC`,
      [workspaceId]
    );
  },

  async upsertToolPolicy(workspaceId: string, channelId: string | null, profile: string): Promise<ToolPolicy> {
    const id = crypto.randomUUID();
    const rows = await query<ToolPolicy>(
      `INSERT INTO tool_policies (id, workspace_id, channel_id, profile, created_at, updated_at)
       VALUES ($1, $2, $3, $4, now(), now())
       ON CONFLICT (workspace_id, COALESCE(channel_id, ''))
       DO UPDATE SET profile = EXCLUDED.profile, updated_at = now()
       RETURNING *`,
      [id, workspaceId, channelId || null, profile]
    );
    return rows[0];
  },
  ...
  async deleteToolPolicy(id: string): Promise<boolean> {
    const res = await query(
      `DELETE FROM tool_policies WHERE id = $1 RETURNING id`,
      [id]
    );
    return res.length > 0;
  }
