...
* **Opt-in workspace/channel tool policy profiles.** Added database-backed `tool_policies` rows and admin API endpoints to scope available agent tools by workspace or channel using the existing `coding`, `research`, `messaging`, and `minimal` profiles, plus explicit `unrestricted`. Unconfigured workspaces and channels remain unrestricted and behave identically to previous releases.
